...
class CoMatch(Trainer, MSELoss, L2Loss, callbacks.InitialCallback, callbacks.TrainCallback):
    ...
    def train_step(self, idx, batch, params: ParamsType, *args, **kwargs):
        ...
        def graph_cs2():
            ...
            anchor = batch_cosine_similarity(sup_gquery.detach(), pos_memory)
            positive = batch_cosine_similarity(sup_gkey.detach(), pos_memory)
        ...
        def graph_cs3():
            ...
            # memory = queue_feats
            pos_memory = torch.cat(self.g_queue_list)
            if params.un_choice_in_queue:
                pos_memory = torch.cat([
                    choice_(un_gquery, 256),
                    choice_(un_gkey, 256),
                    choice_(pos_memory, 256)
                ])
            pos_memory = choice_(pos_memory, self.feature_dim)
        ...
        def strategy21():
            self.exp.add_tag('loss2-1')
            loss = loss_x + loss_u + Lgcs1 * 0.2 + loss_contrast
            return loss

        def strategy22():
            self.exp.add_tag('loss2-2')
            loss = loss_x + loss_u + Lgcs1 * 0.2 + loss_contrast
            return loss
        ...
        def strategy4():
            ...
            loss = loss_x + loss_u + Lgcs1 + loss_contrast
            return loss
