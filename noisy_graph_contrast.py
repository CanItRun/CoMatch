...
class SupContrast(Trainer, MSELoss, L2Loss, callbacks.InitialCallback, callbacks.TrainCallback):
    ...
    def train_step(self, idx, batch, params: ParamsType, *args, **kwargs):
        ...
        unxs, unys = unbatch['xs0'], unbatch['nys']
        ...
        def strategy1():
            ...
            if len(self.queue_list) > 0:
                Lgcs2 = graph_cs3() + (graph_cs2() if params.sup_graph else 0)
                return Lgcs2 + Lce
            return Lce
...
def main():
    ...
    dltrain_x, dltrain_u = get_train_loader2(
        ...
        1024, L=params.n_percls, root=cache_dir(), method='comatch', noisy_ratio=0.2)
