...
def train_one_epoch(epoch,
    ...
    for it in range(n_iters):
        ...
        (unxs, usxs_0, usxs_1), unys = next(dl_u)
        ...
        with torch.no_grad():
            ...
            if epoch > 0 or it > args.queue_batch:  # memory-smoothing
                ...
                un_probs = args.alpha * un_probs + (1 - args.alpha) * torch.mm(A, queue_probs)
            ...
            # update memory bank
            n = bt + btu
            queue_feats[queue_ptr:queue_ptr + n, :] = w_query
            queue_probs[queue_ptr:queue_ptr + n, :] = w_probs
        ...
        Q = torch.mm(un_probs, un_probs.t())
        Q.fill_diagonal_(1)
        ...
        # contrastive loss
        loss_contrast = contrastive_loss2(un_query, un_key, temperature=args.temperature, qk_graph=Q)
